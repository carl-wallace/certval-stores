...
//! provenance checks in `tests.rs` and the only place the trust set is exercised against
//! certificates a part actually presented.
//!
//! Two of them check revocation live and are `#[ignore]`d; see the notes on each. The rest are
//! offline.
...
/// Authenticode digest of `inputs/TrustedTpm.cab`, the cabinet this crate's material came from.
const EXPECTED_CAB_DIGEST: &str =
    "E1F48903EDB3E767681AB2DE837765BBFDA3309DF060F0AA81D44E5BE748A729";
...
struct ValidationInstructions {
    ...
    /// Note this has no effect in this crate: certval is built here without the `revocation`
    /// feature, so no revocation checking is compiled in either way. Carried over from tpm_roots,
    /// where it did distinguish the two, and kept so the cases still read as they were written.
...
impl ValidationInstructions {
    ...
    fn prepare_environment(&mut self) -> certval::Result<()> {
        ...
        // when the publisher shipped it.
        self.cps.set_time_of_interest(
            certval_store_gen::core::published_as_time_of_interest(
        ...
        // The material this crate carries, rather than the CBOR pair tpm_roots embedded. Anchors
        // arrive as DER and the CA store as CBOR, which is the shape a provider has.
        ...
        for (ii, der) in entry.roots.iter().enumerate() {
            ta_source.push(CertFile {
                filename: format!("tpm anchor #{ii}"),
                bytes: der.to_vec(),
            });
        }
...
fn fail_on_new_folders() {
    let trusted_tpm = include_bytes!("../inputs/TrustedTpm.cab");
...
/// The committed cabinet is the one this crate's material was generated from.
///
/// Offline: `CabVerifyParts::new` computes the Authenticode digest without touching the network.
/// Pinned so that swapping the cabinet without regenerating is caught here rather than as a
/// puzzling diff in the store.
#[test]
fn the_committed_cabinet_has_the_expected_digest() {
    use tpm_cab_verify::CabVerifyParts;
    let cursor = std::io::Cursor::new(include_bytes!("../inputs/TrustedTpm.cab"));
    let cvp = CabVerifyParts::new(cursor).expect("the committed cabinet must parse");
    assert_eq!(EXPECTED_CAB_DIGEST, buffer_to_hex(&cvp.digest));
}

/// The committed cabinet's Authenticode signature verifies.
///
/// `#[ignore]` because verification reaches a timestamp authority to settle the signer's
/// revocation status, and a test that fails when a responder is down is a test nobody trusts —
/// the same reason `the_store_is_what_the_committed_cabinet_generates` uses `cab::members` rather
/// than `cab::open_verified`. The signature is checked for real where the material enters the
/// repository, in the refresh. Run it deliberately with `cargo test -- --ignored`.
#[tokio::test]
#[ignore = "reaches a timestamp authority; the refresh is where this is enforced"]
async fn the_committed_cabinet_verifies() {
    use tpm_cab_verify::CabVerifyParts;
    let cursor = std::io::Cursor::new(include_bytes!("../inputs/TrustedTpm.cab"));
